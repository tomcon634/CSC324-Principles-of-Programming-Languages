...
(define (blank? board i) (equal?
                          (vector-ref board i) 0))  ; Whether the given cell is blank.
...
(define (get-constraints puzzle i)
  (check-sets puzzle i
              ...
              (check-sets puzzle i
              (check-sets puzzle i
              (check-sets puzzle i 1-9 1) 2) 3) 4) 5) 6) 7) 8) 9))

(define (check-sets puzzle i curr-set num)
  (if (or (subset? (set num) (row puzzle (to-row i)))
          (subset? (set num) (column puzzle (to-column i)))
          (subset? (set num) (subsquare puzzle (to-subsquare i))))
      (set-remove curr-set num)
      curr-set))
...
(define (set->choice set)
  (if (set-empty? set) (next)
      (if (set-empty? (set-rest set))
          (-< (set-first set))
          (-< (set-first set) (set->choice (set-rest set))))))
...
(define (initialize-constraints board)
  (init-const-helper board 0))

(define (init-const-helper board i)
  (if (< i (vector-length board))
      (if (blank? board i)
          (append (list (list i (get-constraints board i)))
                  (init-const-helper board (+ i 1)))
          (init-const-helper board (+ i 1)))
      empty))
...
(define (with-ordered-constraints-helper board constraints)
  (let ([const (sort-constraints constraints)])
    (cond
      ; If there are no more choices to make, check if the puzzle is actually solved.
      [(equal? const empty)
       (if (solved? board)
           board
           (next))] ; (next) doesnt call next

      ; If the current cell is occupied, move on to the next cell.
      [(not (blank? board (first (first const))))
       (with-ordered-constraints-helper board (rest const))]

      ; Else, the current cell is empty. Make a new choice!
      [else
       (let* ([choice (set->choice (second (first const)))]
              [new-board (vector-set board (first (first const)) choice)])
         (with-ordered-constraints-helper new-board
           (update-constraints (rest const) (first (first const)) choice)))])))
...
(define (update-constraints constraints i choice)
  (update-helper constraints i choice '()))

(define (update-helper constraints i choice new-const)
  (if (> (length constraints) 0)
      (if (or (same-column? i (first (first constraints)))
              (same-row? i (first (first constraints)))
              (same-subsquare? i (first (first constraints))))
          (let ([new (append (list (list (first (first constraints))
                                                  (set-remove (second (first constraints))
                                                              choice))) new-const)])
            (update-helper (rest constraints) i choice new))
            #;(if (set-empty? (set-remove (second (first constraints)) choice))
                (next)
                (update-helper (rest constraints) i choice new))
          (update-helper (rest constraints) i choice
                         (append (list (first constraints)) new-const)))
      new-const))
