...
(define (helper binding ht)
  (hash-set ht (second binding) (hash-ref ht (third binding) (third binding))))
...
|#
(define (fix-first x f)
  (lambda rest-id (apply f (cons x rest-id))))
...
|#
(define (curry-n n f)
  (if (equal? n 1) (lambda (x) (f x))
      (lambda (x) (curry-n (- n 1) (fix-first x f)))))
...
(module+ test
  ...
  (test-equal? "base case" (let ((f1 (curry-n 1 (lambda (x) (equal? x 1)))))
                             (f1 1)) #t)
